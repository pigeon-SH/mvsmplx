...
for cam_id in [4, 28, 52, 76]:
    img_path = f"/home/vclab/8T_SSD1/extractSMPL/MultiviewSMPLifyX/data_smplx/Hi4D/backhug/backhug02/{frame_num}/color/{frame_num}_{cam_id:02d}.jpg"
    ...
    kpts_path = f"/home/vclab/8T_SSD1/extractSMPL/MultiviewSMPLifyX/data_smplx/Hi4D/backhug/backhug02/{frame_num}/keypoints_openpose/{frame_num}_{cam_id:02d}_keypoints.json"
    ...
    for i, kpts_d in enumerate(kpts_data):
        ...
        for kpt in kpts:
            if kpt[-1] > 0.3:
                cv2.circle(img, (int(kpt[0]), int(kpt[1])), radius=8, color=colors[idx], thickness=-1)
    cv2.imwrite(f"test_{frame_num}_{cam_id:02d}.png", img)
