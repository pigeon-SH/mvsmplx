...
def main():
    seqs = ["talk/talk22"] # ["backhug/backhug02", "hug/hug01", "talk/talk22"]
    for seq in seqs:
        gt_root = f"/home/vclab/8T_SSD1/dataset/Hi4D/{seq}/smpl"
        pred_root = f"/home/vclab/8T_SSD1/extractSMPL/MultiviewSMPLifyX/result/1106_temporal_7view/{seq}"
        frames = [int(f.split(".")[0]) for f in sorted(os.listdir(gt_root))]
        NUM_PERSON = 2
        ...
        for frame_name in tqdm(frames):
            ...
            for person_id in range(NUM_PERSON):
                ...
                # load pred
                if frame_name == frames[0] and person_id == 0:
                    betas_0 = torch.tensor(param_pred[person_id]['result']['betas'], dtype=torch.float32)
                elif frame_name == frames[0] and person_id==1:
                    betas_1 = torch.tensor(param_pred[person_id]['result']['betas'], dtype=torch.float32)
